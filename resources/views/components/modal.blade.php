@props(['maxWidth' => 'max-w-md', 'open'=>false])
<div class="w-full " x-data="{ open: {{$open?'true':'false'}} }" x-cloak @keydown.escape.window="open = false">
    ...
    <div x-show="open" x-transition:enter="transition ease-out duration-500"
        ...
        class="w-full z-50 flex justify-center items-center  h-full md:inset-0  mx-auto absolute " style="min-height: 54rem">

        {{-- Modal Container --}}
        <div
            class=" bg-white rounded-lg flex flex-col p-4 mx-auto w-full {{ $maxWidth }} shadow-sm shadow-blue-500 dark:bg-gray-700 border border-blue-500" @click.away="open=false">
