<div class="w-full ">
    ...
    <div class="flex mb-4 select-none">
        <div wire:click="setComponent('recursos.recursos-detail')"
            class="cursor-pointer flex-grow  border-b-2  {{ $componentName == 'recursos.recursos-detail' ? 'text-indigo-500 border-indigo-500' : 'border-gray-300' }}  py-2 text-lg px-1">
            Detalles
        </div>
    ...
    @switch($componentName)
        @case('recursos.recursos-detail')
            @livewire($componentName, ['recurso'=>$recurso, key(uniqid().'det')] )
        @break

        @case('recursos.recurso-proceso')
            <livewire:recursos.provision-proceso :recurso="$recurso" :wire:key="uniqid().'pro'" />
        @break
