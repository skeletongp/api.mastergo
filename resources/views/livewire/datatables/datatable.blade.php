                ...
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg  shadow-sm" style="width: 28rem">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="block w-full py-3 pl-10 text-sm border-gray-300 leading-4 rounded-md shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Buscar en')}} {{ $this->searchableColumns()->map->label->join(', ') }}" type="text" />
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @foreach ($columnGroups as $name => $group)
                    <button wire:click="toggleGroup('{{ $name }}')"
                        ...
                        <span class="flex items-center h-5">{{ isset($this->groupLabels[$name]) ? __($this->groupLabels[$name]) : __('Cambiar :group', ['group' => $name]) }}</span>
                    </button>
                @endforeach
                            ...
                            @foreach($this->columns as $index => $column)
                                ...
                                @elseif($column['type'] === 'checkbox')
                                    @unless($column['hidden'])
                                        <div class="flex justify-center table-cell w-32 h-12 px-6 py-4 overflow-hidden text-xs font-medium tracking-wider text-left text-gray-500 uppercase align-middle border-b border-gray-200 bg-gray-50 leading-4 focus:outline-none">
