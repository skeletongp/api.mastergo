<div>
    <x-modal maxWidth="max-w-4xl" :listenOpen="true">
        <x-slot name="title">
            <span> Editar registro</span>
        </x-slot>
        <x-slot name="button">
            <span data-tooltip-target="editId{{ $user_id }}" class="far fa-pen-square fa-xl text-green-600"></span>
            <x-tooltip id="editId{{ $user_id }}">Editar registro</x-tooltip>
        </x-slot>
        <form wire:submit.prevent="updateUser">
            <div class="flex py-4 justify-end">
                <x-toggle id="logueable{{ $user_id }}" label="Logueable" value="yes" wire:model="loggeable">
                </x-toggle>
            </div>
            <div class="  pb-6 flex items-center space-x-3">
                <div class="w-full">
                    <x-input label="Primer nombre" id="name{{ $user_id }}" wire:model.defer="user.name" />
                    <x-input-error for="user.name"></x-input-error>
                </div>
                <div class="w-full">
                    <x-input label="Apellidos" id="lastname{{ $user_id }}" wire:model.defer="user.lastname" />
                    <x-input-error for="user.lastname"></x-input-error>
                </div>
            </div>
            <div class="  pb-6 flex items-end space-x-3">
                <div class="w-full">
                    <x-input label="Correo Electrónico" id="email{{ $user_id }}" type="email"
                        wire:model.defer="user.email" />
                    <x-input-error for="user.email"></x-input-error>
                </div>
                <div class="w-full">
                    <x-input label="No. Teléfono" id="phone{{ $user_id }}" wire:model.defer="user.phone" />
                    <x-input-error for="user.phone"></x-input-error>
                </div>
            </div>
            <div class="   flex items-end space-x-3">
                <div class="w-full">
                    <label for="avatar{{ $user_id }}" class="flex items-center space-x-4 pb-2 cursor-pointer">
                        ...
                        <input wire:model="avatar" type="file" class="hidden" name="avatar"
                            id="avatar{{ $user_id }}" accept="image/png, image/gif, image/jpeg">
                    </label>
                ...
                @can('Asignar Roles')
                    <div class="w-full">
                        ...
                        <x-select id="role{{ $user_id }}" wire:model.defer="role" class="">
                            <option value=""></option>
            ...
            <div class="py-3 flex justify-between items-center">
                <div>
                    @if ($photo_path)
                        <label for="avatar{{ $user_id }}">
                            <div for="avatar{{ $user_id }}" class="w-12 h-12 rounded-full bg-cover cursor-pointer"
                    ...
                    @else
                        @if (count($user))
                            <label for="avatar{{ $user_id }}">
                                <div for="avatar{{ $user_id }}"
                                    class="w-12 h-12 rounded-full bg-cover cursor-pointer"
