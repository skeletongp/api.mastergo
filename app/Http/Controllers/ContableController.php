...
class ContableController extends Controller
{
    ...
    public function results()
    {
        ...
        $ingresos =
            $ventas = $place->findCount('400-01')->balance;
        ...
        $costos_totales = $place->counts()->where('code', 'like', '500%')->sum('balance');
        $costos_ventas = $place->findCount('500-01')->balance;
        $costos_flete = $place->counts()->whereCode('500-02')->sum('balance');
        $otros_costos = $costos_totales-$costos_ventas-$costos_flete;

        $utilidad = $ingresos - $costos_ventas - $costos_flete - $otros_costos;
